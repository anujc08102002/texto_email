...
public final class DnsTxtRecordParser {
    ...
    private static final Pattern QUOTED_CHUNKS = Pattern.compile("\"([^\"]*)\"");
    ...
    public static String stripQuotesAndWhitespace(String value) {
        ...
        return reconstruct(value).replaceAll("\\s+", " ").trim();
    }

    private static boolean looksFullyQuoted(String value) {
        return value.startsWith("\"") && value.endsWith("\"") && value.indexOf('"', 1) == value.length() - 1;
    }
